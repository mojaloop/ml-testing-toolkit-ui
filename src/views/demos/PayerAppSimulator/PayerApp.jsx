...
class PayerMobile extends React.Component {
  ...
  getStepItems = () => {
    ...
    if (this.state.quoteResponse && this.state.quoteResponse.transferAmount && this.state.quoteResponse.transferAmount.amount) {
      ...
      steps.push({
        title: 'Quote',
        description: <TraceWrap trace={this.state.quoteResponse.trace}><>Payee FSP fee is {payeeFspCurrency} {payeeFspFee}<br />Payee receives {this.state.quoteResponse.transferAmount.currency + ' ' + this.state.quoteResponse.transferAmount.amount}</></TraceWrap>,
        status: 'finish'
      })
    }
