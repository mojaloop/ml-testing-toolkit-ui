  ...
  componentDidMount = async () => {
    ...
    if (getConfig().isAuthEnabled) {
      ...
      storedLogs.data.forEach(log => {
        if (this.activityLogRef.current) { this.activityLogRef.current.appendLog(log) }
        if (this.sequenceDiagramRef.current) { this.sequenceDiagramRef.current.appendLog(log) }
      })
    }
